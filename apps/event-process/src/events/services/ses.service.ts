...
@Injectable()
export class SesService extends EventsService {
  async processSes(events: SesEvent): Promise<any> {
    await this.msgOpsService.checkPostgresConnection();

    const { payload: envelope, account: providerAccount } = events;

    // Subscription confirmation handshake — SES requires a one-time GET to
    // SubscribeURL when the SNS subscription is set up. Without this, no
    // events are ever delivered. We log the URL and DO NOT auto-confirm
    // (deliberate: confirming on inbound traffic would defeat the security
    // purpose of the handshake; ops calls this URL manually after vetting).
    if (envelope?.Type === 'SubscriptionConfirmation') {
      this.formatterUtils.logInfo(
        `[SES] SubscriptionConfirmation received for topic ${envelope.TopicArn}; SubscribeURL=${envelope.SubscribeURL}. Confirm manually.`,
      );
      return { status: 'confirmation_pending', topicArn: envelope.TopicArn, subscribeUrl: envelope.SubscribeURL };
    }
    if (envelope?.Type === 'UnsubscribeConfirmation') {
      this.formatterUtils.logInfo(`[SES] UnsubscribeConfirmation received for topic ${envelope.TopicArn}`);
      return { status: 'unsubscribed', topicArn: envelope.TopicArn };
    }
    if (!envelope?.Message) {
      this.formatterUtils.logInfo(`[SES] Empty SNS envelope`);
      return { status: 'skipped', message: 'Empty envelope' };
    }

    let raw: SesRawEvent;
    try {
      raw = JSON.parse(envelope.Message);
    } catch (err) {
      this.formatterUtils.logInfo(`[SES] Could not JSON.parse SNS Message: ${err}`);
      return { status: 'skipped', message: 'Invalid JSON' };
    }

    const normalized = this.normalizeEvent(raw, envelope);
    if (!normalized) {
      return { status: 'skipped', message: 'Unsupported or contextless event' };
    }

    const uniqueEvents = await this.filterDuplicateEvents([normalized]);
    if (uniqueEvents.length === 0) return { status: 'skipped', message: 'Duplicate event' };
  ...
  // SES surfaces context via mail.tags (multi-value per name). We collapse to
  // SendGrid-style category strings ('account:1', 'message:5') taking the
  // first value of each tag — same convention as our send-email handler emits.
  private normalizeEvent(raw: SesRawEvent, envelope: SnsEnvelope): SesPayload | null {
    const mapped = this.mapEventType(raw.eventType);
    if (!mapped) {
      this.formatterUtils.logInfo(`[SES] Skipping unsupported eventType: ${raw.eventType}`);
      return null;
    }
    const tags = raw.mail?.tags || {};
    const category: string[] = [];
    for (const [k, vs] of Object.entries(tags)) {
      const first = Array.isArray(vs) && vs.length > 0 ? vs[0] : undefined;
      if (first !== undefined) category.push(`${k}:${first}`);
    }

    const contactId = tags.contactId?.[0];
    const email =
      raw.mail?.destination?.[0] ||
      raw.bounce?.bouncedRecipients?.[0]?.emailAddress ||
      raw.complaint?.complainedRecipients?.[0]?.emailAddress ||
      raw.delivery?.recipients?.[0] ||
      '';

    // HARD vs SOFT determined by SES bounceType. Permanent → HARD recipient
    // bounce (mark contact bounced); Transient/Undetermined → SOFT.
    let synthType: string | undefined;
    let bounceClassification: string | undefined;
    if (raw.eventType === SesEventTypes.BOUNCE) {
      bounceClassification = `${raw.bounce?.bounceType}/${raw.bounce?.bounceSubType ?? ''}`;
      synthType = raw.bounce?.bounceType === 'Permanent' ? 'bounce' : 'blocked';
    }

    const tsSource =
      raw.delivery?.timestamp ||
      raw.bounce?.timestamp ||
      raw.complaint?.timestamp ||
      raw.open?.timestamp ||
      raw.click?.timestamp ||
      raw.mail?.timestamp ||
      envelope.Timestamp;
    ...
    return {
      email,
      timestamp: this.parseTimestamp(tsSource),
      event: mapped,
      sesType: raw.eventType,
      category,
      // SNS MessageId is unique per delivery — perfect dedup key.
      ses_event_id: envelope.MessageId,
      ses_message_id: raw.mail?.messageId || '',
      reason:
        raw.bounce?.bouncedRecipients?.[0]?.diagnosticCode ||
        raw.complaint?.complaintFeedbackType ||
        raw.reject?.reason ||
        raw.failure?.errorMessage,
      ip: raw.open?.ipAddress || raw.click?.ipAddress,
      useragent: raw.open?.userAgent || raw.click?.userAgent,
      url: raw.click?.link,
      contactId,
  ...
  private mapEventType(type: string): string | null {
    switch (type) {
      case SesEventTypes.SEND:
        return EventsType.PROCESSED;
      case SesEventTypes.DELIVERY:
        return EventsType.DELIVERED;
      case SesEventTypes.DELIVERY_DELAY:
        return EventsType.DEFERRED;
      case SesEventTypes.BOUNCE:
        return EventsType.BOUNCE;
      case SesEventTypes.COMPLAINT:
        return EventsType.SPAMREPORT;
      case SesEventTypes.OPEN:
        return EventsType.OPEN;
      case SesEventTypes.CLICK:
        return EventsType.CLICK;
      case SesEventTypes.REJECT:
      case SesEventTypes.RENDERING_FAILURE:
        return EventsType.DROPPED;
      case SesEventTypes.SUBSCRIPTION:
        // Subscription event is metadata about list-management, not a delivery
        // event — drop silently.
        return null;
      default:
        return null;
  ...
  private async organizeEventsData(
    ...
    const accountGroups: Record<
      string,
      {
        accountId: number;
        timeZone: string;
        events: { type: string; eventList: SesPayload[] }[];
        eventMap: Map<string, { type: string; eventList: SesPayload[] }>;
        logs: EventLog[];
      }
    > = {};
    ...
    for (const event of events) {
      ...
      if (!messageId || !accountId) {
        this.formatterUtils.logInfo(`[Event-Process] Missing ses context: ${JSON.stringify(event)}`);
        continue;
      }
          ...
          properties: {
            ...
            ...([EventsType.BOUNCE, EventsType.BLOCKED].includes(event.event as EventsType)
              ? {
                  ...
                  ses_type: event.sesType,
                }
              : {}),
          },
          delivered_id: event.ses_message_id || '',
        };
  ...
  private async updateBouncedContacts(accountId: number, events: SesPayload[]): Promise<void> {
  ...
  private async persistBounce(accountId: number, events: SesPayload[], bounceType: 'HARD' | 'SOFT'): Promise<void> {
  ...
  private async filterDuplicateEvents(events: SesPayload[]): Promise<SesPayload[]> {
    const pipeline = this.redisClient.pipeline();
    const eventIds = events.map((e) => e.ses_event_id);
    eventIds.forEach((id) => {
      if (id) pipeline.set(`event:ses:${id}`, '1', 'EX', 10 * 60, 'NX');
    });
    const results = await pipeline.exec();
    return events.filter((event, index) => {
      if (!event.ses_event_id) return true;
      if (results[index][1] !== 'OK') {
        console.log(`[SES] Duplicate event: ${JSON.stringify(event)}`);
        return false;
      }
      return true;
    });
