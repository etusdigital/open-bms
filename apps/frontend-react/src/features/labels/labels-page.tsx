...
import { useLabelsList, useDeleteLabel } from './use-labels';
import { useLabelsColumns } from './labels-columns';
import type { Label } from './types';

const EMPTY_ARRAY: Label[] = [];

interface LabelsPageProps {
  searchParams: ListSearchParams;
}

export default function LabelsPage({ searchParams }: LabelsPageProps) {
  ...
  const query = useLabelsList(searchParams);
  ...
  }, [deleteTarget, deleteLabel, query.data?.data, searchParams.page, setPagination]);

  const columns = useLabelsColumns({ onDelete: handleDelete, canDelete });
  ...
  return (
    <>
      <ListPage.Root>
        <ListPage.Header title={t('labels.pageTitle')}>
          {canCreate && (
        ...
        {isEmpty ? (
          <ListPage.Empty>
            <DataTableEmptyState
              entityName={t('labels.entityNamePlural')}
      ...
      <ConfirmDialog
        ...
        title={t('common.deleteConfirmTitle', { entity: t('labels.entityName') })}
