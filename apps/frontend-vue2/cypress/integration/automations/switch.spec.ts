...
describe('Automations', () => {
  ...
  it('deve existir um switch', () => {
    cy.get('[data-cy=button-edit-1]').should('be.disabled');

    cy.intercept('PATCH', 'http://localhost:5000/automations', {
      id: 1,
      isActive: false,
    });

    cy.get('[data-cy=switch-1]').should('not.be.visible').should('have.attr', 'aria-checked', 'true');
    cy.get('.v-input--selection-controls__input').should('be.visible').click();
    cy.get('[data-cy=modal-success]').should('be.visible').should('have.text', ' Desativar ').click();
