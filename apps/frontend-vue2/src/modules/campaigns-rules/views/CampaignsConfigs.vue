<template>
  <div class="col-12 pt-0">
    <form class="default-filters-messages" @submit.prevent="filterByTitle">
      <div class="default-filters">
        <div class="default-filters__search-input">
          <InputDefault
            :modelValue="title"
            :placeholder="`${$t('input.searchConfig')}`"
            :prependIcon="'search'"
            :keyInput="'title'"
            @click="filterByTitle"
    ...
    <div v-if="campaignsConfigs.length > 0">
      <DataLoader :isLoading="isLoadingCampaignsConfigs" :type="'table-tbody,table-tbody'" class="mt-4" />
      <DataTable
        :headers="headers"
        :items="campaignsConfigs"
        :actions="['duplicate', 'delete']"
        :pagination="pagination"
        :isLoading="isLoadingCampaignsConfigs"
        :pageReference="'campaign-rules-configs'"
        :options="options"
        :rangeStart="rangeStart"
        :rangeFinal="rangeFinal"
        :totalData="pagination.totalItems"
        @delete="confirmDelete"
        @duplicate="createCopyConfig"
        @onChangeOptions="handleOptions"
        @handlePagination="handlePagination"
        @getDataRequest="getCampaignsConfigs"
      />
    </div>
    <div v-if="campaignsConfigs.length === 0 && !isLoadingCampaignsConfigs" class="container-no-results">
      <img src="@/assets/tags_fill.svg" class="mx-1" alt="automations" />
      <p class="font-16 font-title-style">{{ $t('datatable.noConfig') }}</p>
...
export default class CampaignsConfigs extends Vue {
  ...
  campaignsConfigs: Array<CampaignConfigDto> = new Array<CampaignConfigDto>();
  idAccount: any;
  options: any = {
    page: 1,
    sortBy: ['name'],
  ...
  isLoadingCampaignsConfigs = false;
  title: string | (string | null)[] = '';

  headers = [
    { text: this.$t('datatable.name'), value: 'name', sortable: true, width: '80%' },
  ...
  async beforeMount() {
    const storedItemsPerPage = getItemsPerPage('campaignsConfigs');
    if (storedItemsPerPage) {
      this.pagination.itemsPerPage = storedItemsPerPage;
    }
    this.getValuesUrl();
    await this.getCampaignsConfigs();
  }

  async getCampaignsConfigs() {
    if (this.isLoadingCampaignsConfigs) {
      return;
    }

    this.isLoadingCampaignsConfigs = true;
    this.loadingService.show();

    try {
      this.rangeStart = this.pagination.itemsPerPage * (this.pagination.page - 1) + 1;
      const result = await this.campaignRuleService.getCampaignsConfigs({
        ...this.$route.query,
        ...this.pagination,
        title: this.title,
      });
      this.campaignsConfigs = result?.data?.results;
      ...
      this.isLoadingCampaignsConfigs = false;
  ...
  async deleteCampaignConfig() {
    await this.campaignRuleService.deleteCampaignConfig(this.idAccount);
    await this.getCampaignsConfigs();
  }

  confirmDelete(tag: CampaignConfigDto) {
    this.idAccount = tag.id;
    this.modalService.confirm({
      title: this.$t('modal.deleteMessage') as string,
      text: `${this.$t('modal.confirmTag', { tag: tag.name })}`,
      confirmLabel: this.$t('button.delete') as string,
      cancelLabel: this.$t('button.cancel') as string,
      confirmFunction: this.deleteCampaignConfig,
    });
  }
  ...
  async createCopyConfig(config: CampaignConfigDto) {
    if (config.id) {
      this.loadingService.show();
      const response = await this.campaignRuleService.createCopyConfig(config.id);
      this.loadingService.hide();
      if (response && response.data && response.data.id) {
        this.toastService.show({
          type: 'success',
          text: this.$t('modal.configDuplicated') as string,
          leftBorder: false,
        });
        this.$router.push(`campaign-rules-configs/${response.data.id}`);
      }
    }
  }
