<template>
  <div class="col-12 pt-0">
    <form class="default-filters-messages" @submit.prevent="filterByTitle">
      <div class="default-filters">
        <div class="default-filters__search-input">
          <InputDefault
            :modelValue="title"
            :placeholder="`${$t('input.searchRule')}`"
            :prependIcon="'search'"
            :keyInput="'title'"
            @click="filterByTitle"
    ...
    <div v-if="campaignsRules.length > 0">
      <DataLoader :isLoading="isLoadingCampaignsRules" :type="'table-tbody,table-tbody'" class="mt-4" />
      <DataTable
        :headers="headers"
        :items="campaignsRules"
        :actions="['delete']"
        :pagination="pagination"
        :isLoading="isLoadingCampaignsRules"
        :pageReference="'campaign-rules'"
        :options="options"
        ...
        @getDataRequest="getCampaignsRules"
      />
    </div>
    <div v-if="campaignsRules.length === 0 && !isLoadingCampaignsRules" class="container-no-results">
      <img src="@/assets/tags_fill.svg" class="mx-1" alt="automations" />
      <p class="font-16 font-title-style">{{ $t('datatable.noRule') }}</p>
...
export default class CampaignsRules extends Vue {
  ...
  campaignsRules: Array<CampaignConfigDto> = new Array<CampaignConfigDto>();
  idAccount: any;
  options: any = {
    page: 1,
    sortBy: ['name'],
  ...
  async beforeMount() {
    const storedItemsPerPage = getItemsPerPage('campaignsRules');
    if (storedItemsPerPage) {
      this.pagination.itemsPerPage = storedItemsPerPage;
    }
    this.getValuesUrl();
    await this.getCampaignsRules();
  }

  async getCampaignsRules() {
    if (this.isLoadingCampaignsRules) {
      return;
    }

    this.isLoadingCampaignsRules = true;
    this.loadingService.show();

    try {
      this.rangeStart = this.pagination.itemsPerPage * (this.pagination.page - 1) + 1;
      const result = await this.campaignRuleService.getCampaignsRules({
        ...this.$route.query,
        ...this.pagination,
        title: this.title,
      });
      this.campaignsRules = result?.data?.results;
      ...
      this.isLoadingCampaignsRules = false;
  ...
  async deleteCampaignRule() {
    await this.campaignRuleService.deleteCampaignRule(this.idAccount);
    await this.getCampaignsRules();
  }
  ...
  confirmDelete(tag: CampaignConfigDto) {
    ...
    this.modalService.confirm({
      ...
      confirmFunction: this.deleteCampaignRule,
    });
  }
