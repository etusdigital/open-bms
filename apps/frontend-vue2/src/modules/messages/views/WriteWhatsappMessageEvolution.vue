  ...
  <div>
    <v-card class="background-card message-form mt-2">
      <div class="wpp-column">
        <div>
          <label class="label-title font-12">{{ $t('datatable.headerType') }}</label>
          <v-select
            v-model="content.headerType"
            class="type-select"
            :items="headerTypes"
            item-text="label"
            item-value="value"
          ...
          ></v-select>
          <div v-if="content.headerType === 'text'" class="mt-5 text-area-container div-row">
            <input
              class="text-content"
              :placeholder="`${$t('input.headerText')}`"
              id="headerContent"
              name="headerContent"
              v-model="content.headerContent"
              :keyInput="'headerContent'"
              :disabled="isMessageInUse"
            />
            <span
              class="material-symbols-rounded first-picker top-right unfilled-icon font-24 cursor-pointer"
              @click="toggleEmojiContent"
              >sentiment_satisfied</span
            >
            <div class="emoji-box first-picker emoji-picker">
              <VEmojiPicker
          ...
          <div class="div-column gap-5 ds-gray-color mt-5" v-if="content.headerType === 'image'">
            <div class="div-row gap-10 align-items-center">
              ...
              <label
                for="select-file"
                class="select-file-button text-600 font-10 cursor-pointer mb-0"
                :disabled="isMessageInUse"
                >{{ $t('button.chooseFile') }}</label
              >
              <span class="font-12 file-name">{{ fileName }}</span>
              <span
          ...
          <div class="div-column gap-5 ds-gray-color mt-5" v-if="content.headerType === 'video'">
            <div class="div-row gap-10 align-items-center">
              <input
                type="file"
                id="select-file"
          ...
          <div class="text-area-container div-row">
            ...
            ></textarea>
            <span
              class="material-symbols-rounded second-picker top-right unfilled-icon font-24 cursor-pointer"
              @click="toggleEmojiBody"
              >sentiment_satisfied</span
            >
            <div class="emoji-box second-picker emoji-picker">
              <VEmojiPicker
                v-show="showEmojiBody"
                :style="{ width: '380px', height: '200' }"
                labelSearch="Search"
                lang="pt-BR"
                @select="onSelectEmojiPreview($event, 'body')"
              />
            </div>
          </div>
          <label class="label-title font-12 mt-5">{{ $t('create.messageFooter') }}</label>
          <input
            class="text-content"
            :placeholder="`${$t('input.messageContentFooter')}`"
            id="footer"
            name="footer"
            ...
            :disabled="isMessageInUse"
          />
          <label class="label-title font-12 mt-5">{{ $t('datatable.contentType') }}</label>
          <v-select
            v-model="selectTest"
            class="type-select"
          ...
          <div v-if="localMessageValue.whatsappType === 'call-to-action'" class="mt-5">
            <label class="label-title font-12 mt-5">{{ $t('create.buttonText') }}</label>
            <input
              class="text-content"
              :placeholder="`${$t('input.buttonText')}`"
              id="callToActionText"
              name="callToActionText"
              v-model="localMessageValue.callToActionText"
              :keyInput="'callToActionText'"
              :disabled="isMessageInUse"
            />
          </div>
          <div v-if="localMessageValue.whatsappType === 'call-to-action'" class="mt-5">
            <label class="label-title font-12 mt-5">{{ $t('create.buttonURL') }}</label>
            <input
              class="text-content"
              :placeholder="`${$t('input.buttonURL')}`"
              id="url"
              name="url"
              v-model="localMessageValue.url"
              :keyInput="'url'"
              :disabled="isMessageInUse"
            />
          </div>
          ...
          <div class="preview-wpp mt-1">
            <div class="wpp-column">
              ...
              <div class="wpp-wrapper">
                <div class="wpp-text">
                  <div v-if="content.headerType === 'image' && isUploadFile" class="wpp-image">
                    <img :src="messageFile" alt="WhatsApp layout" />
                  </div>
                  <div v-if="content.headerType === 'video' && isUploadFile" class="wpp-image">
                    <video :src="messageFile" controls></video>
                  </div>
                  <p
                    class="text-color-content header pb-3 pr-5"
                    v-if="content.headerType === 'text' && content.headerContent && content.headerContent !== ''"
                  >
                    <b>{{ content.headerContent }}</b>
                  </p>
                  <p class="text-color-content pb-3 pr-5" v-if="content.body === '' || content.body === undefined">
                    {{ $t('create.messageContent') }}
                  </p>
                  <p class="text-color-content pb-3 pr-5" v-else>
                    {{ content.body }}
                  </p>
                  <p class="text-color-content footer pb-2 pr-5" v-if="content.footer && content.footer !== ''">
                ...
                <div class="wpp-link" v-if="localMessageValue.whatsappType === 'call-to-action'">
                  <p
                    class="text-link"
                    v-if="
                      localMessageValue.callToActionText === '' ||
                      localMessageValue.callToActionText === null ||
                      localMessageValue.callToActionText === undefined
                    "
                  >
                    Link
                  </p>
...
import { ContentMessageDto } from '../dtos/content-message.dto';
...
export default class WriteWhatsappMessageEvolution extends Vue {
  ...
  headerTypes = [
    { label: this.$t('title.none'), value: 'none' },
    { label: this.$t('title.text'), value: 'text' },
    { label: this.$t('title.image'), value: 'image' },
    { label: this.$t('title.video'), value: 'video' },
  ];
  ...
  async beforeMount() {
    ...
    const parseContent = JSON.parse(this.localMessageValue.content as string);

    this.selectTest = this.localMessageValue.whatsappType || 'text';
    this.content.headerType = parseContent?.headerType || 'none';

    this.content.body = parseContent?.body || '';
    this.content.footer = parseContent?.footer || '';

    this.content.headerContent = parseContent?.headerContent || '';

    this.messageFile =
      this.content.headerType === 'image' || this.content.headerType === 'video' ? parseContent?.headerContent : '';

    this.fileName = this.getFileName(parseContent?.headerContent || '');
  }
  ...
  closeEmojiPickerIfClickedOutside(event: MouseEvent) {
    const contentPicker = this.$el.querySelector('.emoji-box .first-picker');
    const bodyPicker = this.$el.querySelector('.emoji-box .second-picker');
    const contentButton = this.$el.querySelector('.material-symbols-rounded.first-picker');
    const bodyButton = this.$el.querySelector('.material-symbols-rounded.second-picker');

    if (event.target !== contentPicker && event.target !== contentButton) {
      this.showEmojiContent = false;
    }

    if (event.target !== bodyPicker && event.target !== bodyButton) {
      this.showEmojiBody = false;
    }
  }
  ...
  onChangeFile(event: InputEvent) {
    ...
    reader.onload = () => {
      this.messageFile = reader.result as string;
    };
    ...
    const images = await this.apiService.uploadImages([
      {
        ...
        data: '' + (await this.getBase64(this.file)),
        name: this.file?.name || '',
      },
    ]);
  ...
  async createMessage(status: MessageStatus) {
    ...
    try {
      ...
      if (this.isUploadFile && (this.content.headerType === 'image' || this.content.headerType === 'video')) {
        this.content.headerContent = await this.uploadImage();
      }

      this.localMessageValue.content = JSON.stringify(this.content);
  ...
  toggleEmojiBody() {
    this.showEmojiBody = !this.showEmojiBody;
  }

  onSelectEmojiPreview(emoji: any, input: string) {
    if (input === 'content') {
      const value = `${this.content.headerContent || ''}${emoji.data}`;
      this.content.headerContent = value;
      this.toggleEmojiContent();
    }

    if (input === 'body') {
      const value = `${this.content.body || ''}${emoji.data}`;
      this.content.body = value;
      this.toggleEmojiBody();
    }
  }

  @Watch('file')
  loadFile() {
    this.isUploadFile = !!this.file;
  }
...
.text-color-content.header {
  font-size: 18px;
}
.text-color-content.footer {
  font-size: 14px;
