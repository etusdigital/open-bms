    ...
    <div class="col-12 title-button title-position mb-0 align-items-center">
      <div class="div-column">
        <router-link
          ...
          v-if="/\/([0-9]+)(?=[^\/]*$)/.test($route.path) || $route.path.includes('segments/new')"
        >
      ...
      <button
        v-if="listPath && $store.getters.can('audience:segments_execute')"
        class="v-btn-icon button-create"
        @click="$router.push('/segments/new')"
      >
...
@Component({})
export default class SegmentModule extends Vue {
  ...
  getPageTitle() {
    if (this.$route.path.includes('segments')) {
      this.listPath = true;
      this.title = this.$t('sidebar.segments') as string;
    }
    if (this.$route.path.includes('segments/new')) {
      this.listPath = false;
      this.title = this.$t('button.newSegment') as string;
    }
    if (/\/([0-9]+)(?=[^\/]*$)/.test(this.$route.path)) {
      this.listPath = false;
      this.title = this.$t('title.editSegment') as string;
    }
  }
}
</script>
