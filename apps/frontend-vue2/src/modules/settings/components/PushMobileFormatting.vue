<template>
  <v-card class="mobile-push-format pb-5" :class="{ isMobileSameTemplate }">
    <div class="div-row gap-10 align-center justify-space-between">
      <span class="font-12 text-600 ds-gray-color">{{ $t('input.templateDesktop') }}</span>
      <v-switch :input-value="isMobileSameTemplate" @change="checkTemplate" inset class="mt-0"></v-switch>
    </div>
    <div v-if="!isMobileSameTemplate" class="push-formatting">
      <div class="mt-5 div-more-items">
          ...
          >
            <template v-slot:activator="{ on }">
              ...
              >
                ...
                <div>
                  <span class="material-symbols-rounded icon-up" :class="{ 'icon-dropdown': showPushPosition === true }"
        ...
        <div v-for="(trigger, index) in triggerOptions" :key="trigger.name">
          ...
          <div class="pt-2 div-row hour-select align-center mt-1">
            <input
              ...
              min="1"
              :disabled="trigger.name !== scriptMobileType"
      ...
      <div class="div-first-item div-last-item">
      ...
      <div class="div-first-item div-last-item">
        <div class="d-flex justify-space-between">
          ...
          <span class="font-12 text-400" style="font-size: 12px; color: #d9d9d9"
            >{{ pushMobileStyle.title.length }}/60</span
          >
        </div>
        <input
          v-model="pushMobileStyle.title"
          @input="changeText('title', $event.target.value)"
          class="input-style"
          maxlength="60"
        />
        <label class="label-title-small mt-4">{{ $t('input.subTitle') }}</label>
        <textarea class="input-style" v-model="pushMobileStyle.subTitle"></textarea>
      ...
      <div class="div-first-item div-last-item">
        ...
        <input
          v-model="pushMobileStyle.permissionButtonText"
      ...
      <div class="div-first-item div-last-item">
        ...
        <label class="label-title-small mt-4">{{ $t('input.text') }}</label>
        <input
          v-model="pushMobileStyle.denyButtonText"
      ...
      <div class="div-first-item pb-5">
        ...
        <div class="mt-4 image-area" v-if="showImage">
          <div class="cursor-pointer drop-actions">
                ...
                >
                  ...
                  <div>
                    <span
                      ...
                      :class="{ 'icon-dropdown': showImagePosition === true }"
                    >
                      arrow_drop_down
                    </span>
                  </div>
...
@Component({
  components: { ButtonDefault, InputDefault, AddStepButtonComponent },
  props: ['pushMobileStyle', 'scriptMobileType', 'pushMobileScriptValue', 'isMobileSameTemplate'],
})
export default class PushMobileFormatting extends Vue {
  private readonly toastService = new ToastService();
  private readonly apiService = new ApiService();

  @Prop() pushMobileStyle!: any;
  @Prop() pushMobileScriptValue!: any;
  @Prop() scriptMobileType!: string;
  @Prop({ default: true }) isMobileSameTemplate!: boolean;
  ...
  accessMobileNumber!: number;
  percentMobileNumber!: number;
  inactiveMobileNumber!: number;
  triggerOptions: any = [];
  ...
  imagePositions = [
    { value: this.$t('input.left'), name: 'row-reverse' },
    { value: this.$t('input.right'), name: 'row' },
  ];
  currentState = 'empty';
  logoImage!: any;
  dragging = false;
  ...
  beforeMount() {
    this.selectedPosition =
      this.pushMobileStyle.push.top === 1 ? (this.$t('input.footer') as string) : (this.$t('input.top') as string);
    this.selectedBackgroundColor = this.pushMobileStyle.push.background;
    this.accessMobileNumber =
      this.scriptMobileType === 'access' && this.pushMobileScriptValue.match(/const scriptValue = (\d+);/)
        ? parseInt(this.pushMobileScriptValue.match(/const scriptValue = (\d+);/)[1], 10)
        : 0;
    this.percentMobileNumber =
      this.scriptMobileType === 'percentScroll' && this.pushMobileScriptValue.match(/const scriptValue = (\d+);/)
        ? parseInt(this.pushMobileScriptValue.match(/const scriptValue = (\d+);/)[1], 10)
        : 0;
    this.inactiveMobileNumber =
      this.scriptMobileType === 'inactive' && this.pushMobileScriptValue.match(/const scriptValue = (\d+);/)
        ? parseInt(this.pushMobileScriptValue.match(/const scriptValue = (\d+);/)[1], 10)
        : 0;
    this.triggerOptions = [
      {
        ...
        name: 'access',
        number: this.accessMobileNumber / 1000,
        text: this.$t('input.secLate'),
      },
      {
        value: this.$t('input.scrollPercent'),
        name: 'percentScroll',
        number: this.percentMobileNumber,
        text: this.$t('input.screenScroll'),
      },
      {
        value: this.$t('input.dueInactivity'),
        name: 'inactive',
        number: this.inactiveMobileNumber / 1000,
        text: this.$t('input.secInactive'),
      },
    ];
    this.selectedTrigger = this.triggerOptions.findIndex((x: any) => x.name === this.scriptMobileType);
    this.pushHasBorderRadius = this.pushMobileStyle.push['border-radius'] !== '0px';
    this.pushHasBorder = this.pushMobileStyle.push.border !== 'none';
    this.selectedBorderColor = this.pushMobileStyle.push.border.match(/#([0-9A-Fa-f]{6}|[0-9A-Fa-f]{3})\b/)
      ? this.pushMobileStyle.push.border.match(/#([0-9A-Fa-f]{6}|[0-9A-Fa-f]{3})\b/)[0]
      : '#ffffff';
    this.selectedTextColor = this.pushMobileStyle.textColor.color;
    this.selectedTextColorPermission = this.pushMobileStyle.permissionButton.color;
    this.selectedBackgroundColorPermission = this.pushMobileStyle.permissionButton.background;
    this.selectedBorderColorPermission = this.pushMobileStyle.permissionButton.border.match(
      /#([0-9A-Fa-f]{6}|[0-9A-Fa-f]{3})\b/
    )
      ? this.pushMobileStyle.permissionButton.border.match(/#([0-9A-Fa-f]{6}|[0-9A-Fa-f]{3})\b/)[0]
      : '#ffffff';
    this.buttonBorderRadiusPermission = this.pushMobileStyle.permissionButton['border-radius'] || '0px';
    this.permissionHasBorder = this.pushMobileStyle.permissionButton.border !== 'none';
    this.selectedTextColorDeny = this.pushMobileStyle.denyButton.color;
    this.selectedBackgroundColorDeny = this.pushMobileStyle.denyButton.background;
    this.selectedBorderColorDeny = this.pushMobileStyle.denyButton.border.match(/#([0-9A-Fa-f]{6}|[0-9A-Fa-f]{3})\b/)
      ? this.pushMobileStyle.denyButton.border.match(/#([0-9A-Fa-f]{6}|[0-9A-Fa-f]{3})\b/)[0]
      : '#ffffff';
    this.buttonBorderRadiusDeny = this.pushMobileStyle.denyButton['border-radius'] || '0px';
    this.denyHasBorder = this.pushMobileStyle.denyButton.border !== 'none';
    this.showImage = this.pushMobileStyle.logoStyle.display !== 'none';
    this.selectedImagePosition = /reverse/.test(this.pushMobileStyle.divTitle['flex-direction'])
      ? (this.$t('input.right') as string)
      : (this.$t('input.left') as string);
    this.logoImage = this.pushMobileStyle.logo;
    this.currentState = this.pushMobileStyle.logo.includes('http') || 'empty';
  }

  changePosition(index: number) {
    this.selectedPosition = this.positionOptions[index].value as string;
    this.showPushPosition = false;
    this.$emit('changeProperty', 'push', 'bottom', this.positionOptions[index].name === 'top' ? 1 : '0px');
    this.$emit('changeProperty', 'push', 'top', this.positionOptions[index].name === 'top' ? '0px' : 1);
  }
  ...
  changeImagePosition(index: number) {
    ...
    this.$emit(
      ...
      'divTitle',
      'flex-direction',
      this.pushMobileStyle.divTitle['flex-direction'] === 'row' ? 'row-reverse' : 'row'
    );
  }
  ...
  checkTemplate(value: boolean) {
    this.$emit('isMobileSameTemplate', value);
  }
}
</script>
...
::v-deep.v-card.mobile-push-format {
  box-shadow: none !important;
}

.mobile-push-format {
  height: 725px !important;
  &.isMobileSameTemplate {
