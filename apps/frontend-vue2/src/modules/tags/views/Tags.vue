<template>
  <div class="col-12 pt-0">
    ...
    <div v-if="tags.length > 0">
      <DataLoader :isLoading="isLoadingTags" :type="'table-tbody,table-tbody'" class="mt-4" />
      <DataTable
        :headers="headers"
        :items="tags"
        :actions="['delete']"
        ...
        @getDataRequest="getTags"
      />
    </div>
    <div v-if="tags.length === 0 && !isLoadingTags" class="container-no-results">
      <img src="@/assets/tags_fill.svg" class="mx-1" alt="automations" />
      <p class="font-16 font-title-style">{{ $t('datatable.noTag') }}</p>
...
export default class Tags extends Vue {
  ...
  headers = [
    ...
    {
      text: this.$t('datatable.quantity'),
      value: 'countContacts',
      sortable: false,
      align: 'end',
      width: '10%',
  ...
  async beforeMount() {
    const storedItemsPerPage = getItemsPerPage('tags');
  ...
  async getTags() {
    ...
    try {
      ...
      this.tags = this.tags.map((item) => {
        return {
          ...item,
          countContacts: Vue.filter('formatNumber')(item.countContacts || 0),
        };
      });
  ...
  async deleteTag() {
    await this.tagService.deleteTag(this.idAccount);
    await this.getTags();
  }
  ...
  confirmDelete(tag: TagDto) {
    ...
    this.modalService.confirm({
      ...
      confirmFunction: this.deleteTag,
    });
  }
