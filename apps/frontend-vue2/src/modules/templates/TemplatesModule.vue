    ...
    <div class="col-12 title-button title-position mb-0 align-items-center">
      <div class="div-column">
        <router-link
          ...
          v-if="/\/([0-9]+)(?=[^\/]*$)/.test($route.path) || $route.path.includes('templates/new')"
        >
      ...
      <button
        v-if="listPath && $store.getters.can('messages:create')"
...
@Component({})
export default class TemplatesModule extends Vue {
  ...
  getPageTitle() {
    if (this.$route.path.includes('templates')) {
      this.listPath = true;
      this.title = this.$t('sidebar.templates') as string;
    }
    if (this.$route.path.includes('templates/new')) {
      this.listPath = false;
      this.title = this.$t('button.newTemplate') as string;
    }
    if (/\/([0-9]+)(?=[^\/]*$)/.test(this.$route.path)) {
      this.listPath = false;
      this.title = this.$t('title.editTemplate') as string;
    }
  }
}
</script>
