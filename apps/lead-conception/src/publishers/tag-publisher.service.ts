...
@Injectable()
export class TagPublisherService implements OnModuleInit {
  ...
  async publish(message: Record<string, any>, attributes: Record<string, string> = {}): Promise<void> {
    ...
    await this.publisher.publish({ exchange: EXCHANGES.tags, routingKey: 'tag.process', payload: message, headers });
  }
