...
describe('AppService', () => {
  ...
  describe('Step Type: CONTACT_VALIDATE', () => {
    it('should validate email and update contact if needed', async () => {
      // Arrange
      const validateStep = createMockStep(StepType.CONTACT_VALIDATE, {
        child: [createEndStep()],
      });
      const mockContact = createMockContact({ hasEmail: false });
      const mockLeadStateMessage = createMockLeadStateMessage({
        automation: {
          id: 50,
          type: 'email',
        ...
        account: {
          id: 1,
          customFields: [],
          accountConfigs: { api_key: 'test-key' },
        },
      });
      const messageId = 'msg-validate';

      redisClient.exists.mockResolvedValue(0);
      mocks.mockEmailValidationProvider.emailChecker.mockResolvedValue({ result: 'deliverable' });
      ...
      // Assert
      expect(result).toBe('message-id-123');
      expect(mocks.mockEmailValidationProvider.emailChecker).toHaveBeenCalledWith(mockContact.email, 'test-key');
      expect(mocks.mockMsgopsService.updateContact).toHaveBeenCalledWith({ has_email: true }, mockContact.id, mockContact.accountId);
    });
  });
  ...
  describe('Step Type: ACTIVE_CAMPAIGN', () => {
    it('should create contact in ActiveCampaign', async () => {
      // Arrange
      const activeCampaignStep = createMockStep(StepType.ACTIVE_CAMPAIGN, {
        settings: {
          apiKey: 'ac-api-key',
      ...
      const mockLeadStateMessage = createMockLeadStateMessage({
        automation: {
          id: 50,
          type: 'email',
          title: 'Test ActiveCampaign',
          steps: [activeCampaignStep],
        },
        contact: mockContact,
      });
      const messageId = 'msg-ac';

      redisClient.exists.mockResolvedValue(0);
      mocks.mockMsgopsService.findContactById.mockResolvedValue(mockContact);
      mocks.mockActiveCampaignProvider.createContact.mockResolvedValue({ id: 123 });
      ...
      // Assert
      expect(result).toBe('message-id-123');
      expect(mocks.mockActiveCampaignProvider.createContact).toHaveBeenCalledWith(mockContact, activeCampaignStep.settings);
    });

    it('should throw BadRequestException when contact not found in ACTIVE_CAMPAIGN', async () => {
      // Arrange
      const activeCampaignStep = createMockStep(StepType.ACTIVE_CAMPAIGN, {
        settings: {
          listId: 5,
      ...
      const mockLeadStateMessage = createMockLeadStateMessage({
        automation: {
          id: 50,
          type: 'email',
          title: 'Test AC Contact Not Found',
          steps: [activeCampaignStep],
        },
      });
      ...
      // Act & Assert
      await expect(service.receiveMessage(mockLeadStateMessage, 'msg-ac-notfound', null)).rejects.toThrow(BadRequestException);
    });
  });
