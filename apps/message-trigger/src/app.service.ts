...
export class AppService {
  ...
  private async contactValidate(leadStateMessage: LeadStateMessage) {
    const { contact } = leadStateMessage;
    const apiKey = leadStateMessage.account.accountConfigs.api_key;
    const emailChecker = await this.emailValidationProvider.emailChecker(contact.email, apiKey);

    const emailIsValid = emailChecker.result === EmailVerify.DELIVERABLE ? true : false;
    if (contact.hasEmail !== emailIsValid) {
      await this.msgopsService.updateContact({ has_email: emailIsValid }, contact.id, contact.accountId);
    }

    return leadStateMessage;
  }
  ...
  private async processActiveCampaign(leadStateMessage: LeadStateMessage, step: Step) {
    const contact = await this.msgopsService.findContactById(leadStateMessage.contact.id, leadStateMessage.contact.accountId, ['customFields']);
    if (!contact) {
      throw new BadRequestException(`[${leadStateMessage.id}] Contact not found.`);
    }
    return await this.activeCampaignProvider.createContact(contact, step.settings);
  }
