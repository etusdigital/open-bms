  ...
  providers: [
    {
      ...
      useFactory: (sparkpost: SparkPostHandler, sendgrid: SendGridHandler) => {
        const registry = new EmailProviderRegistry();
        registry.register(sparkpost);
        registry.register(sendgrid);
        return registry;
      },
      inject: [SparkPostHandler, SendGridHandler],
    },
    EmailProviderRouter,
  ],
  exports: [EMAIL_PROVIDER_REGISTRY_TOKEN, EmailProviderRouter],
})
