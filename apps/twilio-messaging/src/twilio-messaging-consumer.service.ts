...
// /campaign vira de campaign-packer (Onda 4); /single vira de msgops-api
// (Semana 5+). Ambos preservados como código no controller, sem queue AMQP
// até o publisher correspondente migrar — ver docs/migration/evo-952-wave-2-decisions.md §1/§5.
const BINDINGS: QueueBinding[] = [
  {
    exchange: EXCHANGES.sms,
    routingKey: 'sms.send',
    queue: 'twilio-messaging.sms.send',
    bridgePath: '/internal/sms/automation',
  },
];

@Injectable()
export class TwilioMessagingConsumerService {
  ...
  async stop(): Promise<void> {
    await Promise.all(
      this.consumers.map((c) => c.shutdown().catch((err) => console.error('[twilio-messaging] consumer stop:', err))),
    );
  }
}
