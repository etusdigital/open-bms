...
| ---- | ------------ | -------------------------------------------------------- |
| 1    | ✅ Concluída | `worktree-whatsapp-cloud-meta-evohub` · commit `8f807b7` |
| 2    | ⏳ Pendente  |                                                          |
| 3    | ⏳ Pendente  |                                                          |
| 4    | ⏳ Pendente  |                                                          |
| 5    | ⏳ Pendente  |                                                          |
...
**Worktree em uso:** `.claude/worktrees/whatsapp-cloud-meta-evohub` (a partir da base `7baa233`).
**Desvio do spec:** migrations vivem em `apps/msgops-api/src/migrations/` (não em `packages/database/src/migrations/`) — esse é o padrão real do monorepo. Timestamp ajustado para `1781000000000` (> última migration existente `1780000000000`).
...
5. **Excluir arquivos**:
   - `rm apps/send-whatsapp/src/providers/evolution.provider.ts`
   - `rm -rf apps/msgops-api/src/handlers/evolution/`

6. **`apps/msgops-api/src/modules/messages/messages.service.ts`**:
   - Remover `approveEvolution` (linhas 283-364).
   - Remover ambos blocos `if (env.WHATSAPP_PROVIDER === 'twilio')` e refatorar para usar novo `whatsappModeResolver`.
   - Se Twilio é usado para SMS/Email em outro contexto, mover para serviço separado.

7. **`apps/send-whatsapp/src/app.service.ts`**:
   - Remover `EvolutionProvider`.
   - Remover leitura de `whatsapp_number_id`/`whatsapp_access_token` de `accounts_configs`.

8. **Limpar ENVs** em `.env.example`, `docker-compose.yml`, scripts.

9. **Frontend**: remover/reescrever páginas em `apps/frontend-react/src/routes/_authenticated/_layout/messages/whatsapp/` que dependiam do schema Evolution.
