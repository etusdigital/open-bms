...
import { ExternalQueryStep, FieldsType, GenerateDepsV2, GenerateResult, SegmentDtoLike, TagLike } from './types';
...
// from a multi-account scope (`account_id IN (...)`) using the caller-
// resolved `siblingAccounts` list. Behaviorally equivalent to the
// original `SegmentQueryBuilderProvider.generateSegmentQueryV2` — output
// SQL parses identically; trailing whitespace may differ due to prettier
// normalisation on extraction.
//
// SECURITY WARNING: same as V1 — `step.*` fields are interpolated
// directly into SQL. Trusted-input contract; see generator-v1.ts header.
export function generateSegmentQueryV2(tag: TagLike, segmentDto: SegmentDtoLike, deps: GenerateDepsV2): GenerateResult {
  const timeZoneValue = deps.timeZone || 'UTC';
  if (!deps.siblingAccounts) {
    throw new Error(
      `generateSegmentQueryV2: siblingAccounts missing for accountId=${tag.accountId}. ` +
        `An account routed to V2 must have an entry in V2_SIBLING_ACCOUNTS_BY_ACCOUNT_ID.`,
    );
  }
  const siblings = deps.siblingAccounts.join(',');
      ...
      switch (step.type) {
        case 'interation': {
          ...
          if (!step.custom_times_value || parseInt(step.custom_times_value, 10) === 1) {
            ...
            let queryPageView = `SELECT DISTINCT email
                            ...
                            WHERE account_id IN (${siblings})
                            AND event_name = '${parseEventType(step.event)}'
            ...
            const clickhouseQuery = `SELECT email
                            FROM user_events_daily_v3 FINAL
                            WHERE event_name = '${parseEventType(step.event)}'
                            AND account_id IN (${siblings})
        ...
        case 'automation_state':
          query += ` ct.id IN (SELECT contact_id
                        ...
                        WHERE account_id = ${tag.accountId}
                        AND created_at_date >= (CURRENT_DATE AT TIME ZONE '${timeZoneValue}' - interval '${step.time} day')`;
          if (step.automation) {
